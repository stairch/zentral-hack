          ...
          <div className="space-y-8 text-foreground">
            {text.sections.map((section) => (
              <section key={section.title}>
                <h2 className="text-2xl font-bold border-b-2 border-yellow pb-2">{section.title}</h2>
                ...
                {section.title.includes('8.') || section.title.includes('Privacy') ? (
                  <p className="mt-4">
                    <a href="/datenschutz" className="text-violet hover:underline font-semibold">
                      {language === 'de' ? 'Zur Datenschutzerklärung' : 'Read the privacy policy'}
                    </a>
                  </p>
                ) : null}
                {'footnote' in section ? <p className="mt-4 text-sm text-muted-foreground">{section.footnote}</p> : null}
              </section>
            ))}
          </div>
        </div>
