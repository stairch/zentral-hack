...
async function handlePost(req: AuthenticatedRequest) {
  try {
    ...
    const result = await query(
      ...
      [name, description || null, baseTemplateId || 'standard', subject, content, ctaText || null, ctaUrl || null, footerNote || null, req.user!.userId]
    );
    ...
    await query(
      `UPDATE email_templates
       SET name = COALESCE($1, name),
           ...
           updated_at = NOW()
       WHERE id = $9`,
      [name, description, baseTemplateId, subject, content, ctaText || null, ctaUrl || null, footerNote || null, id]
    );
