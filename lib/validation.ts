...
export const SignupSchema = z.object({
  firstName: z.string().min(1, 'Vorname erforderlich').max(50).regex(/^[a-zA-ZäöüßÄÖÜ\s-]+$/, 'Ungültiger Vorname'),
  lastName: z.string().min(1, 'Nachname erforderlich').max(50).regex(/^[a-zA-ZäöüßÄÖÜ\s-]+$/, 'Ungültiger Nachname'),
  email: emailSchema,
...
export const TwoFAVerifySchema = z.object({
  email: emailSchema,
  code: z.string().length(6, 'Verificationscode muss 6 Zeichen lang sein').regex(/^[A-F0-9]+$/, 'Ungültiger Verificationscode'),
});
...
export const SponsorContactSchema = z.object({
  ...
  email: emailSchema,
  phone: z.string().regex(/^[+\d\s\-()]+$/, 'Ungültige Telefonnummer').max(20).optional(),
